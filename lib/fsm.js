var _, EventEmitter, Debug, debug, slice, NEXT_TRANSITION, NEXT_HANDLER, HANDLING, HANDLED, NO_HANDLER, TRANSITION, INVALID_STATE, DEFERRED, NEW_FSM, utils, Fsm, out$ = typeof exports != 'undefined' && exports || this;
_ = require('lodash');
...
out$.Fsm = Fsm = (function(){
  ...
  prototype.exec = function(inputType){
    ...
    debug("exec: %s::%s", this.state, inputType);
    if (!this.inExitHandler) {
      ...
      if (current && (states[current][inputType] || states[current]['*'] || this['*'])) {
  ...
  prototype.task = function(name){
    ...
    task.add = function(txt, fn){
      ...
      debug("(%s): add %d", name, this.fns.length);
      i = this.fns.length;
    ...
    task.next = function(){
      ...
      this.i++;
      this.running++;
      this.emit('running', this.msg = this.msgs[i], i, this.fns.length);
      fn(function(err, res){
        var end;
        task.running--;
        if (err) {
