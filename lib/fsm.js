...
Fsm = (function(){
  ...
  prototype.exec = function(cmd){
    ...
    if (!this.inExitHandler && (state = this.state)) {
      ...
      do_exec = function(fn, handler, path){
        ...
        this$.debug("exec(%s) called:ret (%s)", handler, typeof ret === 'object' ? 'object' : ret);
        emit_obj.ret = ret;
  ...
  prototype.task = function(scope, concurrency, name, cb){
    ...
    task.next = function(){
      ...
      fn.call(task.scope, function(err, res){
        var end, ref$, ref1$;
        task.running--;
        if (err) {
          task._paused = true;
...
Empathy = {
  derivitave: function(name, version){
    if (version) {
      return Semver.satisfies(version, this._derivitaves[name]);
    } else {
  ...
  'extend.initialize': function(){
    ...
    _.each(this.derivitaves, function(d, k){
      return task.push("checking for " + k, function(done){
        ...
        return d(function(v){
          if (v) {
            this$._derivitaves[k] = v;
            this$.debug("found derivitave " + k + "@" + v);
          }
          return done(void 8, v);
        });
      });
    });
    this.on('derivitave:remove', function(){
      return this.debug.todo("go through each one and remove the derivitave version from the extended function");
    });
    this.on('derivitave:add', function(){
      return this.debug.todo("go through each one and add the derivitave versions to the extended function list if it's not already");
    });
    this.on('state:added', function(state){
      return this.debug.todo("calculate the derivitaves");
    });
    task.end(function(){
      var transition, exec;
      this.emit('derivitaves:calculated');
      transition = function(e){
        var this$ = this;
        return _.each(this._derivitaves, function(v, derivitave){
          var d;
          if (e.fromState && (d = this$.states[e.fromState][derivitave + ":onexit"])) {
            d.apply(this$, e.args);
          }
          if (d = this$.states[e.toState][derivitave + ":onenter"]) {
            return d.apply(this$, e.args);
          }
        });
      };
      exec = function(e){
        ...
        return _.each(this._derivitaves, function(v, derivitave){
          var d, dd;
          if ((d = this$.states[derivitave + ":" + this$.state]) && (dd = d[e.type])) {
            dd.apply(this$, e.args);
          }
          if ((d = this$.cmds) && (dd = d[derivitave + ":" + e.type])) {
            return dd.apply(this$, e.args);
          }
        });
      };
