var ToolShed, Debug, _, EventEmitter, debug, slice, NEXT_TRANSITION, NEXT_HANDLER, HANDLING, HANDLED, NO_HANDLER, TRANSITION, INVALID_STATE, DEFERRED, NEW_FSM, utils, Fsm, out$ = typeof exports != 'undefined' && exports || this;
...
out$.Fsm = Fsm = (function(){
  ...
  prototype.error = function(err){
    var states, estate;
    states = this.states;
    if (estate = states[this.state]._onError) {
      return this.states[this.state]._onError.call(this);
    } else {
  ...
  prototype.emit = function(eventName){
    ...
    doEmit = function(){
      ...
      switch (eventName) {
      case HANDLING:
        this$.debug("executing: (%s:%s)", this$.state, args[1].type);
        break;
      case HANDLED:
        this$.debug("executed: (%s:%s)", this$.state, args[1].type);
        break;
      case INVALID_STATE:
        this$.debug("bad transition: (%s !-> %s)", args[1].state, args[1].attemptedState);
        break;
      case TRANSITION:
        this$.debug("transition: (%s -> %s)", args[1].fromState, args[1].toState);
        break;
      default:
        this$.debug("emit: (%s): %s", eventName, JSON.stringify(args[1]));
      }
  ...
  prototype.exec = function(cmd){
    ...
    if (!this.inExitHandler) {
      ...
      if (current && (states[current][cmd] || states[current]['*'] || this['*'])) {
        handlerName = states[current][cmd] ? cmd : '*';
  ...
  prototype.transition = function(newState){
    var oldState;
    this.debug("transition %s -> %s", this.state, newState);
  ...
  prototype.processQueue = function(type){
    ...
    filterFn = type === NEXT_TRANSITION
      ? function(item){
        return item.type === NEXT_TRANSITION && typeof this.states[this.state][item.cmd] !== 'undefined';
      }
      : type === DEFERRED
        ? function(item){
  ...
  prototype.task = function(name){
    ...
    task.choke = function(txt, fn){
      ...
      self.debug("task(%s): choke %d", name, this.fns.length);
    ...
    task.add = function(txt, fn){
      ...
      self.debug("task(%s): add %d", name, this.fns.length);
      i = this.fns.length;
      this.fns.splice(i, 0, fn);
      this.msgs.splice(i, 0, txt);
      task.done = false;
      this.next();
      return task;
    };
    task.push = function(txt, fn){
      ...
      self.debug("task(%s): push %d", name, this.fns.length);
      i = this.fns.length;
      this.fns.push(fn);
      this.msgs.push(txt);
      task.done = false;
    ...
    task.next = function(){
      ...
      fn.call(self, function(err, res){
        ...
        } else if (typeof task.cb === 'function') {
