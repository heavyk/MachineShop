...
var Fs, Path, Url, assert, spawn, mkdirp, Rimraf, printf, EventEmitter, _, nw_version, v8_version, HOME_DIR, v8_mode, Debug, debug, Fiber, Future, Environment, scan, parse, rm, isDirectory, unquote, isQuoted, stripEscapeCodes, mkdir, exists, stat, readdir, readFile, writeFile, exec, searchDownwardFor, recursive_hardlink, debug_fn, get_obj_path, set_obj_path, out$ = typeof exports != 'undefined' && exports || this;
Fs = require('fs');
Path = require('path');
Url = require('url');
...
Debug = function(namespace){
  ...
  if (HOME_DIR && !process.env.DEBUG) {
    ...
    debug.error = do_append_msg('error', "[ERROR] " + namespace + ": ", function(msg, channel, prefix, postfix, _write){
      if (!process.env.DEBUG_NO_DEBUGGER) {
        debugger;
      }
...
exec = function(cmd, opts, cb){
  var deep_err, err, silent, cmds, p, stdout, stderr;
...
get_obj_path = function(path, obj, split){
  var paths, i, _obj;
  if (typeof path === 'string') {
    ...
    if (typeof split !== 'string') {
      throw new Error("arg{3:split} is supposed to be a string");
    }
  ...
  if (typeof obj !== 'function' && typeof obj !== 'object') {
    throw new Error("arg{2:obj} is supposed to be of type Object or Function");
  }
  i = 0;
  _obj = obj;
  while (i < paths.length && _obj) {
    _obj = _obj[paths[i++]];
  }
  return _obj;
};
set_obj_path = function(path, obj, val, split, subsplit){
  var paths, subobj, i, ii, subpaths, i$, ref$, len$, p, pp, _subobj;
  if (typeof path === 'string') {
    if (typeof split === 'undefined') {
      split = '.';
    }
  ...
  if (typeof subsplit === 'undefined') {
    subsplit = ',';
  }
  if (typeof subsplit !== 'string') {
    throw new Error("arg{5:subsplit} is supposed to be a string");
  }
  if (paths.length) {
    subobj = obj;
    while (paths.length) {
      ...
      if (~(i = path.indexOf('{')) && ~(ii = path.lastIndexOf('}'))) {
        ...
        for (i$ = 0, len$ = (ref$ = subpaths.split(subsplit)).length; i$ < len$; ++i$) {
          p = ref$[i$];
